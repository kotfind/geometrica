use types::core::{Circ, Line, Pt, Transformation, Value};

pub(crate) trait Transformable {
    /// Applies [t] if possible and resturns Some(...), returns None otherwise.
    ///
    /// Transformation can be applied to geometry objects (lines, points, circles) and cannot be
    /// applied to others.
    fn transform(self, t: Transformation) -> Option<Self>
    where
        Self: Sized;
}

impl Transformable for Value {
    fn transform(self, t: Transformation) -> Option<Self> {
        match self {
            Value::Pt(pt) => pt.map(|pt| pt.transform(t).into()),
            Value::Line(line) => line.map(|line| line.transform(t).into()),
            Value::Circ(circ) => circ.map(|circ| circ.transform(t).into()),
            Value::Array(vec) => vec.map(|vec| {
                vec.into_iter()
                    .filter_map(|v| v.transform(t))
                    .collect::<Vec<_>>()
                    .into()
            }),
            Value::Bool(_) | Value::Int(_) | Value::Real(_) | Value::Str(_) => None,
        }
    }
}

impl Transformable for Pt {
    fn transform(self, t: Transformation) -> Option<Self> {
        Some(self * t.zoom + t.offset)
    }
}

impl Transformable for Line {
    fn transform(self, t: Transformation) -> Option<Self> {
        let Line { mut p1, mut p2 } = self;

        p1 = p1.transform(t).expect("points are transformable");
        p2 = p2.transform(t).expect("points are transformable");

        Some(Line { p1, p2 })
    }
}

impl Transformable for Circ {
    fn transform(self, t: Transformation) -> Option<Self> {
        let Circ { mut o, mut r } = self;

        o = o.transform(t).expect("points are transformable");
        r *= t.zoom;

        Some(Circ { o, r })
    }
}

/// Creates a [Transformation] that transform rectangle
///     (-1, -1),
///     ( 1, -1),
///     ( 1,  1),
///     (-1,  1)
/// into rectangle that exactly fits into this rectangle
///     (origin.x,          origin.y),
///     (origin.x + size.x, origin.y),
///     (origin.x,          origin.y + size.y),
///     (origin.x + size.x, origin.y + size.y)
/// so that their centers are the same
pub(crate) fn from_bounds(origin: Pt, size: Pt) -> Transformation {
    Transformation {
        offset: Pt {
            x: origin.x + size.x / 2.0,
            y: origin.y + size.y / 2.0,
        },
        zoom: size.y.min(size.x) / 2.0,
    }
}

#[cfg(test)]
mod test {
    use super::*;

    #[test]
    fn transform_origin() {
        let offset = Pt { x: 123.0, y: 456.0 };

        let t = Transformation { offset, zoom: 42.0 };

        let pt = Pt { x: 0.0, y: 0.0 };

        assert_eq!(pt.transform(t).expect("points are transformable"), offset);
    }

    #[test]
    fn transform_zoom() {
        let t = Transformation {
            offset: Pt { x: 0.0, y: 0.0 },
            zoom: 42.0,
        };

        let pt = Pt { x: 1.0, y: 2.0 };

        assert_eq!(
            pt.transform(t).expect("points are transformable"),
            Pt { x: 42.0, y: 84.0 }
        );
    }

    #[test]
    fn transform_complex() {
        let t = Transformation {
            offset: Pt { x: 4.0, y: 5.0 },
            zoom: 6.0,
        };

        let pt = Pt { x: 7.0, y: 8.0 };

        assert_eq!(
            pt.transform(t).expect("points are transformable"),
            Pt {
                x: 7.0 * 6.0 + 4.0,
                y: 8.0 * 6.0 + 5.0,
            }
        );
    }

    #[test]
    fn from_bounds_horizontal_fit() {
        let t = from_bounds(Pt { x: 100.0, y: 200.0 }, Pt { x: 100.0, y: 500.0 });

        assert_eq!(
            t,
            Transformation {
                offset: Pt { x: 150.0, y: 450.0 },
                zoom: 50.0
            }
        )
    }

    #[test]
    fn from_bounds_vertical_fit() {
        let t = from_bounds(Pt { x: 200.0, y: 100.0 }, Pt { x: 500.0, y: 100.0 });

        assert_eq!(
            t,
            Transformation {
                offset: Pt { x: 450.0, y: 150.0 },
                zoom: 50.0
            }
        )
    }
}
