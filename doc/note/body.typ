...
Процесс выполнения кода происходит следующим образом (смотри также @execution_process):

- Из строкового представления код парсится в `Vec<Statement>`. Дальнейшие шаги
    описаны для каждого `Statement` отдельно

- Если очередное выражение (`Statement`) является командой (`Command`; то есть
    оно может менять структуру дерева), то оно обрабатывается особым образом.
    Например, команда `set` изменяет значение вершины, `eval` вычисляет значение
    выражения и т.д.

- Иначе, если очередное выражение (`Statement`) является объявлением
    (`FunctionDefinition` или `ValueDefinition`), то в списке функций или вершин
    соответственно, создается новый элемент. Тело функции или значение выражения,
...
- `Expr` компилируется в `CExpr` (от Compiled `Expr`). На этом этапе происходит
    ...
    выражения. Так, если структура `Expr` очень похожа на синтаксис встроенного
    языка программирования и удобна для ввода/ вывода (превращения в строку и
    обратно), то структура `CExpr` оптимизирована для простоты вычислений.
...
Server может быть запущен либо вручную, либо один из клиентов.
...
в они достаточно ограничены. Во-первых, сам синтаксис языка ограничен: так в
обоих языках нельзя комментировать код, создавать локальные переменные
(отсутствует конструкция `let`), а в GeoGebra нельзя объявлять новые функции.
Во-вторых, ограничены возможности по вводу скриптов, а именно: весь скрипт
отображается в одной небольшой ячейке, что усложняет работу с большими
скриптами; один скрипт может создать только один объект или семейство объектов
(то есть, например, объявить десять разных точек в одном скрипте нельзя).
Наконец, в языках отсутствуют "продвинутые" команды, такие как `delete`.
...
можно дорабатывать, в тех направлениях, в которых он проигрывает конкурентам.
